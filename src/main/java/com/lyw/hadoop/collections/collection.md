...
## 二叉树 <font size=5px>一个节点最多有2个子树。节点定义（1个key,2个子节点）</font>
  - 二叉树分类
    ...
    - 平衡二叉树（AVL） <font size='4px' color='gray'>平衡二叉树是空树或者它的左右两个子树的高度差绝对值不大于1，并且左右两个子树也是平衡二叉树</font>
      - 特性即满足平衡二叉树条件</br>
           ...
           3. 左右两个子树都是平衡二叉树
    - 2-3树<font color=gray>区别二叉树，是由普通2节点和3节点构成的树。所以定义为2-3树（有事有序二叉树）</br>
             1个普通2节点有一个key，2个子节点（也是2-3节点）</br>
             1个3节点定义：有2个key，3个子节点（也是2-3节点），</font> 
      - 插入原则：（寻找到叶子节点操作）
          1. newKey先按有序二叉树查找插入位置，如果node1节点2个key都有值（即是一个3-node节点），当查找到node1时需要该节点分裂
            先比较newKey、key1、key2大小，然后将中间值的key分裂为其他2个key的父节点，并将这个中间key的值放在
            node1.parent的另一个key上。如果node1节点还有空间，就将该newKey放入到该节点上
          2. newKey先按有序二叉树查找插入位置，如果node1是一个2-node节点，就将node1包装为一个3-node节点
    - 红黑树 <font color=red>是特殊的2-3树，红色节点是隶属于黑色父节点。平衡标准是以节点颜色位判断依据</font>
      - 满足红黑树条件
           1. 是一棵平衡二叉树
           2. 根节点是黑色
           2. 节点颜色是红色或者黑色
